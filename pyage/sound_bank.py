...
class SoundBank(metaclass=PySingleton):
    ...
    cache_size: int = 1024
    """
    by default, all sounds generated by a sound player get cached internally
    to prevent the sound stopping when its object goes out of scope. This
    number indicates the amount of sounds that get cached internally before
    the :class:`~pyage.sound_player.SoundPlayer` cleans its cache from
    sounds that are currently not playing. Modifying this value will only
    effect all sound players that were created after the value was changed.
    Set it to a value <= 0 to disable cache cleaning. Doing that will result in
    the sounds being freed only when the sound player itself goes out of scope.
    """
    ...
    def create_sound_player(self) -> SoundPlayer:
        ...
        return SoundPlayer(self.cache_size)
